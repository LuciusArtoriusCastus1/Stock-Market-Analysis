...
def extract_data_from_table(table):
    ...
    data_price = body.find_all('td', class_='td-right')
    data = [data_price[i].text for i in range(1, len(data_price), 3)]
    prices = [data_price[i].text for i in range(2, len(data_price), 3)]
    ...
    return data_name, names, rank, data, prices, todays, countries


def process_data(data_name, names, rank, data, prices, todays, countries):
    return [
        {
...
def generate_plots(data, data_name, url):
    page = url.split('/')[-1]
...
def parse_url(url):
    ...
    (data_name, names, rank, data,
     prices, todays, countries) = extract_data_from_table(table)
    data = process_data(data_name, names, rank,
                        data, prices, todays, countries)
    images = generate_plots(data, data_name, url)
